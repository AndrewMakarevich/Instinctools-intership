(function preEventsAddon() {
  ...
  function setPreEventFunctionTimeout(callback, timeout, timeBeforeExicute, type) {
    setTimeout(() => {
      callback();
      if (type && type === 'daily') {
        return setPreEventFunctionTimeout(callback, 1000 * 60 * 60 * 24 - timeBeforeExicute);
      }
      return setPreEventFunctionTimeout(callback, 1000 * 60 * 60 * 24 * 7 - timeBeforeExicute);
    }, timeout);
  }
  function setPreEventFunction(foundedEvent, timeBeforeExicute, callback) {
    if (foundedEvent) {
      if (foundedEvent.eventType !== 'repeat') {
        ...
        const millisecondToTheNextDay = currentDate.getTimezoneOffset() - startOfTheNextDay;

        if (foundedEvent.date === 'daily') {
          callback();
          setPreEventFunctionTimeout(callback, 1000 * 60 * 60 * 24 - millisecondToTheNextDay - timeBeforeExicute, timeBeforeExicute, 'daily');
          return;
        }

        if (new Date().getDay() === foundedEvent.date.getDay()) {
          callback();
          setPreEventFunctionTimeout(callback, 1000 * 60 * 60 * 24 * 7 - millisecondToTheNextDay - timeBeforeExicute, timeBeforeExicute);
          return;
        }
