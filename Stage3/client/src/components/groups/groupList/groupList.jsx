...
const GroupList = () => {
  ...
  const [fetchUsers, usersAreLoading] = useDelayFetching(() => dispatch(getGroups(queryParams)), 200);
  ...
  return (
    <article className={listStyles["group-list-wrapper"]}>
      ...
      <PaginationLine count={groupReducer.count} page={queryParams.page} limit={queryParams.limit} setPage={(page) => { setQueryParams({ ...queryParams, page }) }} />
    </article>
  )
};
