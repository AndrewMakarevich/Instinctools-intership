...
describe('Group service: group creating', () => {
  ...
  test('Trying to create a group with incorrect params', async () => {
    await expect(createIncorrectTestGroup()).rejects.toThrow(
      "Group validation failed: groupName: Group's name doesn't match required pattern, groupTitle: Group's title doesn't match required pattern"
    );
  });

  test('Trying to create a group with ununique group-name', async () => {
    await createCorrectTestGroup();
    await expect(createCorrectTestGroup()).rejects.toThrow(
...
describe('Group service: group deleting', () => {
  ...
  test('Trying to delete an unexisting group using incorrect id', async () => {
    await expect(GroupService.deleteGroup('boo')).rejects.toThrow(
      'Incorrect group id'
    );
  });
  ...
  test('Trying to delete an unexisting group using correct id', async () => {
    await expect(
      GroupService.deleteGroup('6245718f0e677453a6312f93')
    ).rejects.toThrow("Group you try to delete doesn't exists");
  });
});
...
describe('Group service: group editing', () => {
  test('Trying to edit an existing group with correct params', async () => {
    ...
    await GroupService.editGroup(
      group._id,
      alterCorrectGroupObj.groupName,
      alterCorrectGroupObj.groupTitle
    );
  ...
  test('Trying to edit an existing group by setting ununique group-name', async () => {
    ...
    await expect(
      GroupService.editGroup(group._id, alterCorrectGroupObj.groupName)
    ).rejects.toThrow('Group name must be unique');
  });

  test('Trying to edit an unexisting group using incorrect id', async () => {
    await expect(GroupService.editGroup('foo', 'Fifth-group')).rejects.toThrow(
      'Incorrect group id'
    );
  });
  ...
  test('Trying to edit an unexisting group using correct id', async () => {
    await expect(
      GroupService.editGroup('624581d8abeb4d8cbbb43c6d', 'Fifth-group')
    ).rejects.toThrow("Group you try to modify doesn't exists");
  });
});
