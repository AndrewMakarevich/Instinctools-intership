...
const { correctUserObj, alterCorrectUserObj, incorrectUserObj } = require("./consts.mock");
...
async function createCorrectTestUser(ownUsername, ownEmail) {
  ...
  const testUser = await UserService.createUser(ownUsername || username, firstName, lastName, ownEmail || email);
...
describe("User service: user creating", () => {
  test("Correct user creating", async () => {
    ...
    const createdUser = await UserService.getUser("username", testUser.user.username);
  ...
  test("Trying to create user with incorrect params", async () => {
    await expect(createIncorrectTestUser())
      .rejects
      .toThrow(`User validation failed: username: Username does't match required pattern, firstName: First name is too long, email: Path \`email\` is invalid (${incorrectUserObj.email}).`);
  });

  test("Trying to create user with ununique username", async () => {
    await createCorrectTestUser();
    await expect(createCorrectTestUser())
      .rejects
  ...
  test("Trying to create user with ununique email", async () => {
    ...
    await expect(createCorrectTestUser("AndrewTheSecond"))
      .rejects
...
describe("User service: user searching", () => {
  ...
  test("Checking pagination in method getUsers", async () => {
    await createCorrectTestUser();
    await createAlterCorrectTestUser();
    const { user: thirdUser } = await createCorrectTestUser("AndrewTheThird", "andrew3@mail.ru");
...
describe("User service: user editing", () => {
  ...
  test("Trying to edit an existing user with ununique email param", async () => {
    ...
    const editUserIncorrectly = async () => {
      await UserService.editUser(secondUser._id, undefined, undefined, undefined, firstUser.email)
    };
